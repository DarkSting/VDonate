...
const setComplainWatched = async(req,res)=>{
    ...
    try{
        ...
        await ComplainModel.findOneAndUpdate({_id:complainID},{checked:true});
...
const findComplain = async(req,res,next)=>{
   ...
   await ComplainModel.findOne({$or:[{refNo:refNo},{checkedBy:checkedBy}]}).then(result=>{
    return res.status(201).json(result);
   }).catch(error=>{
...
const updateComplain = async(req,res,next)=>{
      ...
      const{ 
        ...
        if(updatedDes!==undefined){
            ...
            await ComplainModel.findOneAndUpdate({$or:[{refNo:refNo},{checkedBy:checkedBy}]},{description:updatedDes}).then(result=>{
                console.log(result)
            }).catch(
                err =>{
