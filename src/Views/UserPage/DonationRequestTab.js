...
export default function DonationRequestTab(props){
    ...
    const handleSignUp = (arr)=>{
        ...
        const post = async(arr)=>{
           ...
           await Axios.post('/user/makeDonationRequest', data).then(res=>{
            console.log(res)
...
<>
            {/*form holder*/}
                    ...
                    <FormControl sx={{ m: 1, width:'60%' }} >
                        <InputLabel id="demo-simple-select-error-label">Donation Type</InputLabel>
                        <Select
                    ...
                    <FormHelperText sx={{color:props.hover}}>Choose your donation type</FormHelperText>
      </FormControl>
                    {isBloodTypeNull!=true ?(<Typography variant="body2" sx={{color:'red', marginTop:"10px"}}>{descErr}</Typography>):
                           (<Typography variant="body2" sx={{}}>{descErr}</Typography>)
                        }

                    <TextBox id="outlined-multiline-flexible" label="Description" multiline maxRows={10} borderColor="red" HoverBorderColor={props.hover} onChange={(e)=>{
                    ...
                    <CardActions sx={{display:'flex',justifyContent:'center',alignItems:'center',marginTop:'20px'}}>
                        <Button size="Large" sx={btnprop(props)} onClick={()=>{
